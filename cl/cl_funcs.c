 ...
 */
int cl_addstr(SCR *sp, const char *str, size_t len) {
  size_t oldy, oldx;
  int iv;
  ...
  if (!F_ISSET(sp, SC_SCR_EXWROTE) && oldy == RLNO(sp, LASTLINE(sp)) &&
      IS_SPLIT(sp)) {
    iv = 1;
    (void)standout();
  }
 ...
 */
int cl_attr(SCR *sp, scr_attr_t attribute, int on) {
  CL_PRIVATE *clp;
 ...
 */
int cl_baud(SCR *sp, unsigned long *ratep) {
  CL_PRIVATE *clp;
 ...
 */
int cl_clrtoeol(SCR *sp) { return (clrtoeol() == ERR); }
 ...
 */
int cl_cursor(SCR *sp, size_t *yp, size_t *xp) {
  /*
 ...
 */
int cl_ex_adjust(SCR *sp, exadj_t action) {
  CL_PRIVATE *clp;
  int cnt;
  ...
  case EX_TERM_SCROLL:
    ...
    if (clp->cuu1 != NULL)
      ...
      (void)tputs(tgoto(clp->cup, 0, LINES - 2), 1, cl_putchar);
    else
      return (0);
  /* FALLTHROUGH */
  case EX_TERM_CE:
 ...
 */
void cl_imctrl(SCR *sp, imctrl_t action) {
 ...
 */
int cl_keyval(SCR *sp, scr_keyval_t val, CHAR_T *chp, int *dnep) {
  CL_PRIVATE *clp;
 ...
 */
int cl_move(SCR *sp, size_t lno, size_t cno) {
 ...
 */
int cl_refresh(SCR *sp, int repaint) {
  CL_PRIVATE *clp;
 ...
 */
int cl_rename(SCR *sp, char *name, int on) {
  GS *gp;
  CL_PRIVATE *clp;
  char *ttype;
 ...
 */
int cl_suspend(SCR *sp, int *allowedp) {
  struct termios t;
  CL_PRIVATE *clp;
  size_t oldy, oldx;
  int changed;
