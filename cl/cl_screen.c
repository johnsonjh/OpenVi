 ...
 */
int cl_screen(SCR *sp, u_int32_t flags) {
  CL_PRIVATE *clp;
  GS *gp;
  ...
  if (LF_ISSET(SC_EX)) {
    ...
    if (F_ISSET(sp, SC_EX) && clp->cup != NULL)
      tputs(tgoto(clp->cup, 0, O_VAL(sp, O_LINES) - 1), 1, cl_putchar);
  } else {
    if (cl_vi_init(sp))
      return (1);
    F_CLR(clp, CL_IN_EX);
...
int cl_quit(GS *gp) {
  ...
  if (F_ISSET(clp, CL_STDIN_TTY) && F_ISSET(clp, CL_SCR_VI_INIT) &&
      cl_vi_end(gp))
    rval = 1;

  /* Really leave ex mode. */
  if (F_ISSET(clp, CL_STDIN_TTY) && F_ISSET(clp, CL_SCR_EX_INIT) &&
      cl_ex_end(gp))
    rval = 1;
...
static int cl_vi_init(SCR *sp) {
  ...
  if (!F_ISSET(clp, CL_STDIN_TTY) || !isatty(STDOUT_FILENO)) {
    msgq(sp, M_ERR, "Standard input and standard output must be a terminal");
    return (1);
  }
...
fast:
  if (tcsetattr(STDIN_FILENO, TCSADRAIN | TCSASOFT, &clp->ex_enter)) {
    if (errno == EINTR)
      goto fast;
    msgq(sp, M_SYSERR, "tcsetattr");
    return (1);
 ...
 */
int cl_getcap(SCR *sp, char *name, char **elementp) {
  size_t len;
  char *t;

  if ((t = tigetstr(name)) != NULL && t != (char *)-1 &&
      (len = strlen(t)) != 0) {
 ...
 */
static void cl_freecap(CL_PRIVATE *clp) {
  free(clp->el);
  clp->el = NULL;
  free(clp->cup);
  clp->cup = NULL;
 ...
 */
static int cl_putenv(char *name, char *str, unsigned long value) {
  char buf[40];
