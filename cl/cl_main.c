...
int main(int argc, char *argv[]) {
  ...
  if ((OG_D_STR(gp, GO_TERM) = OG_STR(gp, GO_TERM) = strdup(ttype)) == NULL)
    openbsd_err(1, NULL);
 ...
 */
static CL_PRIVATE *cl_init(GS *gp) {
  CL_PRIVATE *clp;
  int fd;
 ...
 */
static void term_init(char *ttype) {
  int err;
...
static void h_int(int signo) { cl_sigint = 1; }

static void h_term(int signo) { cl_sigterm = signo; }

static void h_winch(int signo) { cl_sigwinch = 1; }
...
int sig_init(GS *gp, SCR *sp) {
  ...
  if (sp == NULL) {
    if (setsig(SIGHUP, &clp->oact[INDX_HUP], h_term) ||
      ...
      openbsd_err(1, NULL);
  } else if (setsig(SIGHUP, NULL, h_term) || setsig(SIGINT, NULL, h_int) ||
             setsig(SIGQUIT, NULL, h_int) || setsig(SIGTERM, NULL, h_term) ||
 ...
 */
static int setsig(int signo, struct sigaction *oactp, void (*handler)(int)) {
