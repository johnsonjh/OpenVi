...
int cl_term_init(SCR *sp) {
  ...
  for (tkp = c_tklist; tkp->name != NULL; ++tkp) {
    if ((t = tigetstr(tkp->ts)) == NULL || t == (char *)-1)
      continue;
    if (seq_set(sp, tkp->name, strlen(tkp->name), t, strlen(t), tkp->output,
                strlen(tkp->output), SEQ_COMMAND, SEQ_NOOVERWRITE | SEQ_SCREEN))
      return (1);
  }
  ...
  for (tkp = m1_tklist; tkp->name != NULL; ++tkp) {
    ...
    if (!strcmp(t, "\b"))
      continue;
    if (tkp->output == NULL) {
      if (seq_set(sp, tkp->name, strlen(tkp->name), t, strlen(t), NULL, 0,
                  SEQ_INPUT, SEQ_NOOVERWRITE | SEQ_SCREEN))
        return (1);
    } else if (seq_set(sp, tkp->name, strlen(tkp->name), t, strlen(t),
                       tkp->output, strlen(tkp->output), SEQ_INPUT,
  ...
  if (F_ISSET(qp, SEQ_FUNCMAP))
    (void)cl_pfmap(sp, qp->stype, qp->input, qp->ilen, qp->output, qp->olen);
  return (0);
}
 ...
 */
int cl_fmap(SCR *sp, seq_t stype, CHAR_T *from, size_t flen, CHAR_T *to,
            size_t tlen) {
 ...
 */
static int cl_pfmap(SCR *sp, seq_t stype, CHAR_T *from, size_t flen, CHAR_T *to,
                    size_t tlen) {
  size_t nlen;
  char *p, key_name[64];

  (void)snprintf(key_name, sizeof(key_name), "kf%d", atoi(from + 1));
  if ((p = tigetstr(key_name)) == NULL || p == (char *)-1 || strlen(p) == 0)
    p = NULL;
  if (p == NULL) {
  ...
  nlen =
      snprintf(key_name, sizeof(key_name), "function key %d", atoi(from + 1));
  if (nlen >= sizeof(key_name))
    nlen = sizeof(key_name) - 1;
  return (seq_set(sp, key_name, nlen, p, strlen(p), to, tlen, stype,
                  SEQ_NOOVERWRITE | SEQ_SCREEN));
}
 ...
 */
int cl_optchange(SCR *sp, int opt, char *str, unsigned long *valp) {
  CL_PRIVATE *clp;
 ...
 */
int cl_omesg(SCR *sp, CL_PRIVATE *clp, int on) {
  struct stat sb;
  char *tty;
  ...
  if (on) {
    if (chmod(tty, sb.st_mode | S_IWGRP) < 0) {
      if (sp != NULL)
        msgq(sp, M_SYSERR, "messages not turned on: %s", tty);
      return (1);
    }
  } else if (chmod(tty, sb.st_mode & ~S_IWGRP) < 0) {
    if (sp != NULL)
      msgq(sp, M_SYSERR, "messages not turned off: %s", tty);
    return (1);
  }
  return (0);
}
 ...
 */
int cl_ssize(SCR *sp, int sigwinch, size_t *rowp, size_t *colp, int *changedp) {
  struct winsize win;
  size_t col, row;
  int rval;
  char *p;
  ...
  if (sigwinch) {
     ...
     */
    if (sp != NULL && row == O_VAL(sp, O_LINES) &&
        col == O_VAL(sp, O_COLUMNS)) {
      if (changedp != NULL)
        *changedp = 0;
      return (0);
    }
 ...
 */
int cl_putchar(int ch) { return (putchar(ch)); }
