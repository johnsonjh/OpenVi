...
static input_t cl_read(SCR *, u_int32_t, CHAR_T *, size_t, int *,
                       struct timeval *);
 ...
 */
int cl_event(SCR *sp, EVENT *evp, u_int32_t flags, int ms) {
  struct timeval t, *tp;
  CL_PRIVATE *clp;
  ...
  clp = CLP(sp);
retest:
  if (LF_ISSET(EC_INTERRUPT) || cl_sigint) {
    if (cl_sigint) {
      cl_sigint = 0;
  ...
  switch (cl_read(sp, LF_ISSET(EC_QUOTED | EC_RAW), clp->ibuf,
                  sizeof(clp->ibuf), &nr, tp)) {
  case INP_OK:
    evp->e_csp = clp->ibuf;
    evp->e_len = nr;
 ...
 */
static input_t cl_read(SCR *sp, u_int32_t flags, CHAR_T *bp, size_t blen,
                       int *nrp, struct timeval *tp) {
...
tty_retry:
  ...
  if (LF_ISSET(EC_QUOTED | EC_RAW) && !tcgetattr(STDIN_FILENO, &term1)) {
    ...
    if (LF_ISSET(EC_QUOTED)) {
      ...
      (void)tcsetattr(STDIN_FILENO, TCSASOFT | TCSADRAIN, &clp->vi_enter);
  }
