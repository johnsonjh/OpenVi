 ...
 */
FREF *file_add(SCR *sp, CHAR_T *name) {
  GS *gp;
  FREF *frp, *tfrp;
 ...
 */
int file_init(SCR *sp, FREF *frp, char *rcv_name, int flags) {
  EXF *ep;
  RECNOINFO oinfo;
  struct stat sb;
  size_t psize;
  ...
  if (LF_ISSET(FS_OPENERR) || oname == NULL || !exists) {
    ...
    if (fd == -1 || fstat(fd, &sb) == -1 ||
      ...
      msgq(sp, M_SYSERR, "Unable to create temporary file");
      if (fd != -1) {
        close(fd);
        (void)unlink(tname);
      }
    ...
    if (!S_ISREG(sb.st_mode))
      msgq_str(sp, M_ERR, oname, "Warning: %s is not a regular file");
  }
  ...
  /* Open a db structure. */
  if ((ep->db =
           dbopen(rcv_name == NULL ? oname : NULL, O_NONBLOCK | O_RDONLY,
                  S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP | S_IROTH | S_IWOTH,
                  DB_RECNO, &oinfo)) == NULL) {
    msgq_str(sp, M_SYSERR, rcv_name == NULL ? oname : rcv_name, "%s");
    /*
     * !!!
  ...
  if (LF_ISSET(FS_SETALT))
    set_alt_name(sp, sp->frp == NULL || F_ISSET(sp->frp, FR_TMPFILE)
                         ? NULL
                         : sp->frp->name);
  ...
  if (rcv_name == NULL && !O_ISSET(sp, O_READONLY))
    switch (file_lock(sp, oname, &ep->fcntl_fd, ep->db->fd(ep->db), 0)) {
    case LOCK_FAILED:
      F_SET(frp, FR_UNLOCKED);
      break;
    case LOCK_UNAVAIL:
      readonly = 1;
      msgq_str(sp, M_INFO, oname, "%s already locked, session is read-only");
      break;
    case LOCK_SUCCESS:
      break;
    }
...
oerr:
  ...
  return (open_err ? file_init(sp, frp, rcv_name, flags | FS_OPENERR) : 1);
}
 ...
 *      try and open.
 */
static int file_spath(SCR *sp, FREF *frp, struct stat *sbp, int *existsp) {
  CHAR_T savech;
  size_t len;
  int found;
  ...
  for (found = 0, p = t = O_STR(sp, O_PATH);; ++p)
    if (*p == ':' || *p == '\0') {
      if (t < p - 1) {
        savech = *p;
        *p = '\0';
        len = snprintf(path, sizeof(path), "%s/%s", t, name);
        if (len >= sizeof(path))
          len = sizeof(path) - 1;
        *p = savech;
        if (!stat(path, sbp)) {
...
static void file_cinit(SCR *sp) {
  ...
  if (gp->c_option != NULL && !F_ISSET(sp->frp, FR_NEWFILE)) {
    ...
    if (ex_run_str(sp, "-c option", gp->c_option, strlen(gp->c_option), 1, 1))
      return;
    gp->c_option = NULL;
 ...
 */
int file_end(SCR *sp, EXF *ep, int force) {
  FREF *frp;
 ...
 */
int file_write(SCR *sp, MARK *fm, MARK *tm, char *name, int flags) {
  ...
  if (!LF_ISSET(FS_FORCE | FS_APPEND) && !O_ISSET(sp, O_WRITEANY)) {
    ...
    if ((!noname || F_ISSET(frp, FR_NAMECHANGE)) && !stat(name, &sb)) {
  ...
  if (stat(name, &sb))
    ...
    if (noname && !LF_ISSET(FS_FORCE | FS_APPEND) &&
      ...
      msgq_str(sp, M_ERR, name,
               LF_ISSET(FS_POSSIBLE)
                   ? "%s: file modified more recently than this copy; use ! to "
                     "override"
  ...
  oflags = O_CREAT | O_WRONLY | (LF_ISSET(FS_APPEND) ? O_APPEND : O_TRUNC);
  ...
  if (!noname && file_lock(sp, NULL, NULL, fd, 0) == LOCK_UNAVAIL)
    msgq_str(sp, M_ERR, name, "%s: write lock was unavailable");
  ...
  if (rval) {
    if (!LF_ISSET(FS_APPEND))
      msgq_str(sp, M_ERR, name, "%s: WARNING: FILE TRUNCATED");
    return (1);
  }
  ...
  case NEWFILE:
    ...
    break;
  case OLDFILE:
    msgstr = LF_ISSET(FS_APPEND) ? "%s: appended: %'lu lines, %'lu characters"
  ...
  s = buf;
  if (len >= sp->cols) {
    for (s = buf, t = buf + strlen(p);
         s < t && (*s != '/' || len >= sp->cols - 3); ++s, --len)
      ;
    if (s == t)
      s = buf;
    else {
 ...
 */
static int file_backup(SCR *sp, char *name, char *bname) {
  struct dirent *dp;
  struct stat sb;
  DIR *dirp;
  EXCMD cmd;
   ...
   */
  if (cmd.argc != 1) {
    msgq_str(sp, M_ERR, bname, "%s expanded into too many file names");
    (void)close(rfd);
    return (1);
  }
  ...
  if (version) {
    ...
    if (bp == NULL)
      goto err;
    for (t = bp, slash = NULL, p = cmd.argv[0]->bp; p[0] != '\0'; *t++ = *p++)
      if (p[0] == '%') {
        if (p[1] != '%')
          *t++ = '%';
      } else if (p[0] == '/')
  ...
  if (stat(wfname, &sb) == 0) {
    if (!S_ISREG(sb.st_mode)) {
      msgq_str(sp, M_ERR, bname, "%s: not a regular file");
      goto err;
    }
    if (sb.st_uid != getuid()) {
 ...
 */
static void file_comment(SCR *sp) {
  recno_t lno;
  size_t len;
  char *p;
 ...
 */
int file_m1(SCR *sp, int force, int flags) {
  EXF *ep;
  ...
  if (F_ISSET(ep, F_MODIFIED)) {
    if (O_ISSET(sp, O_AUTOWRITE)) {
      ...
      msgq(sp, M_ERR,
           LF_ISSET(FS_POSSIBLE) ? "File modified since last complete write; "
                                   "write or use ! to override"
                                 : "File modified since last complete write; "
                                   "write or use :edit! to override");
      return (1);
    }
  }
...
int file_m3(SCR *sp, int force) {
  ...
  if (F_ISSET(sp->frp, FR_TMPEXIT) && ep->refcnt <= 1 && !force) {
    msgq(sp, M_ERR, "File is a temporary; exit will discard modifications");
    return (1);
  }
  return (0);
}
...
int file_aw(SCR *sp, int flags) {
  ...
  if (O_ISSET(sp, O_READONLY)) {
    msgq(sp, M_INFO, "File readonly, modifications not auto-written");
    return (1);
  }
 ...
 */
void set_alt_name(SCR *sp, char *name) {
  free(sp->alt_name);
  if (name == NULL)
    sp->alt_name = NULL;
 ...
 */
lockr_t file_lock(SCR *sp, char *name, int *fdp, int fd, int iswrite) {
  ...
  return (LOCK_FAILED);
#else
  return (flock(fd, LOCK_EX | LOCK_NB) ? errno == EAGAIN || errno == EWOULDBLOCK
                                             ? LOCK_UNAVAIL
                                             : LOCK_FAILED
                                       : LOCK_SUCCESS);
