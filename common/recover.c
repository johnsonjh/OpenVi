...
int rcv_tmp(SCR *sp, EXF *ep, char *name) {
  ...
  for (p = name; *p; ++p)
    if (*p == '\n') {
      msgq(sp, M_ERR, "Files with newlines in the name are unrecoverable");
      goto err;
    }
  ...
  if ((ep->rcv_path = strdup(path)) == NULL) {
    ...
    (void)unlink(path);
  err:
    msgq(sp, M_ERR, "Modifications not recoverable if the session fails");
    return (1);
  }
...
int rcv_init(SCR *sp) {
  ...
  if (ep->rcv_mpath == NULL) {
    ...
    sp->gp->scr_busy(sp, "Copying file for recovery...", BUSY_ON);
    if (ep->db->sync(ep->db, R_RECNOSYNC)) {
      msgq_str(sp, M_SYSERR, ep->rcv_path, "Preservation failed: %s");
 ...
 */
int rcv_sync(SCR *sp, u_int flags) {
  EXF *ep;
  int fd, rval;
  char *dp, buf[1024];
  ...
  if (F_ISSET(ep, F_MODIFIED)) {
    ...
    if (ep->db->sync(ep->db, R_RECNOSYNC)) {
      ...
      msgq_str(sp, M_SYSERR, ep->rcv_path, "File backup failed: %s");
      return (1);
    }
...
int rcv_mailfile(SCR *sp, int issync, char *cp_path) {
  ...
  len = snprintf(buf, sizeof(buf), "%s%s\n%s%s\n%s\n%s\n%s%s\n%s%s\n%s\n%s\n\n",
                 ...
                 "Reply-To: root", "From: root (OpenVi recovery program)",
                 "To: ", pw->pw_name, "Subject: OpenVi saved the file ", p,
  ...
  len = snprintf(buf, sizeof(buf), "%s%.24s%s%s%s%s%s%s%s%s%s%s%s%s%s%s\n\n",
                 ...
                 " was editing a file named ", t, " on the machine ", host,
  ...
  if (len > sizeof(buf) - 1) {
  lerr:
    msgq(sp, M_ERR, "Recovery file buffer overrun");
    goto err;
  }
...
#define FMTCOLS 60
  ...
  if (issync) {
    ...
    if (close(fd)) {
    werr:
      msgq(sp, M_SYSERR, "Recovery file");
      goto err;
    }
  }
  return (0);
...
int rcv_list(SCR *sp) {
  ...
  for (found = 0; (dp = readdir(dirp)) != NULL;) {
    ...
    if (fgets(file, sizeof(file), fp) == NULL ||
        ...
        (t = strchr(path, '\n')) == NULL) {
      msgq_str(sp, M_ERR, dp->d_name, "%s: malformed recovery file");
      goto next;
    }
    *p = *t = '\0';
     ...
     */
    errno = 0;
    if (stat(path + sizeof(VI_PHEADER) - 1, &sb) && errno == ENOENT) {
...
int rcv_read(SCR *sp, FREF *frp) {
  ...
  for (found = requested = 0; (dp = readdir(dirp)) != NULL;) {
    if (strncmp(dp->d_name, "recover.", 8))
      continue;
    if ((size_t)snprintf(recpath, sizeof(recpath), "%s/%s", rp, dp->d_name) >=
        sizeof(recpath))
      continue;
    ...
    if (rcv_gets(file, sizeof(file), fd) == NULL ||
        ...
        strncmp(path, VI_PHEADER, sizeof(VI_PHEADER) - 1) ||
        (t = strchr(path, '\n')) == NULL) {
      msgq_str(sp, M_ERR, recpath, "%s: malformed recovery file");
      goto next;
    }
    *p = *t = '\0';
    ++found;
     ...
     */
    (void)fstat(fd, &sb);
    if (recp == NULL || timespeccmp(&rec_mtim, &sb.st_mtim, <)) {
      p = recp;
      t = pathp;
  ...
  if (found) {
    ...
    if (found > requested)
      msgq(sp, M_INFO, "There are other files for you to recover");
  }
 ...
 */
char *rcv_gets(char *buf, size_t len, int fd) {
  int nr;
  char *p;
...
void rcv_email(SCR *sp, int fd) {
  ...
  if (_PATH_SENDMAIL[0] != '/' || stat(_PATH_SENDMAIL, &sb) == -1)
    msgq_str(sp, M_SYSERR, _PATH_SENDMAIL, "not sending email: %s");
  else {
    /*
     * !!!
