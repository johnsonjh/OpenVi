 ...
 */
int opts_init(SCR *sp, int *oargs) {
  ARGS *argv[2], a, b;
  OPTLIST const *op;
  u_long v;
  int optindx;
...
#define OI(indx, str)                                                          \
  ...
  for (op = optlist, optindx = 0; op->name != NULL; ++op, ++optindx)
    ...
    case OPT_NUM:
      ...
      break;
    case OPT_STR:
      if (O_STR(sp, optindx) != NULL &&
          o_set(sp, optindx, OS_DEF | OS_NOFREE | OS_STRDUP, O_STR(sp, optindx),
                0))
        goto err;
      break;
    default:
...
err:
  msgq(sp, M_ERR, "Unable to set default %s option", optlist[optindx].name);
  return (1);
}
 ...
 */
int opts_set(SCR *sp, ARGS *argv[], char *usage) {
  enum optdisp disp;
  enum nresult nret;
  OPTLIST const *op;
  OPTION *spo;
  ...
  for (rval = 0; argv[0]->len != 0; ++argv) {
    ...
    for (sep = NULL, equals = qmark = 0, p = name = argv[0]->bp;
         (ch = *p) != '\0'; ++p)
      if (ch == '=' || ch == '?') {
        if (p == name) {
          if (usage != NULL)
            msgq(sp, M_ERR, "Usage: %s", usage);
          return (1);
        }
        sep = p;
        if (ch == '=')
    ...
    if ((op = opts_search(name)) == NULL && name[0] == 'n' && name[1] == 'o') {
      turnoff = 1;
      name += 2;
      op = opts_search(name);
    }
     ...
     * argument.
     */
    if (!equals && !qmark && argv[1]->len == 1 && argv[1]->bp[0] == '?') {
      ++argv;
      qmark = 1;
    }
    ...
    case OPT_1BOOL:
      ...
      if (F_ISSET(op, OPT_NOUNSET) && turnoff) {
        msgq_str(sp, M_ERR, name, "set: the %s option may not be turned off");
        rval = 1;
        break;
      }

      if (equals) {
        msgq_str(sp, M_ERR, name, "set: [no]%s option doesn't take a value");
        rval = 1;
        break;
      }
      if (qmark) {
      ...
      if ((op->func != NULL && op->func(sp, spo, NULL, &turnoff)) ||
    ...
    case OPT_NUM:
      if (turnoff) {
        msgq_str(sp, M_ERR, name, "set: %s option isn't a boolean");
        rval = 1;
        break;
      }
      if (qmark || !equals) {
      ...
      if (!isdigit(sep[0]))
        goto badnum;
      if ((nret = nget_uslong(&value, sep, &endp, 10)) != NUM_OK) {
        p = msg_print(sp, name, &nf);
        t = msg_print(sp, sep, &nf2);
        switch (nret) {
        case NUM_ERR:
          msgq(sp, M_SYSERR, "set: %s option: %s", p, t);
          break;
        case NUM_OVER:
          msgq(sp, M_ERR, "set: %s option: %s: value overflow", p, t);
          break;
        case NUM_OK:
        case NUM_UNDER:
          abort();
      ...
      badnum:
        p = msg_print(sp, name, &nf);
        t = msg_print(sp, sep, &nf2);
        msgq(sp, M_ERR, "set: %s option: %s is an illegal number", p, t);
        if (nf)
          FREE_SPACE(sp, p, 0);
        if (nf2)
          FREE_SPACE(sp, t, 0);
      ...
      if (F_ISSET(op, OPT_NOZERO) && value == 0) {
        msgq_str(sp, M_ERR, name, "set: the %s option may never be set to 0");
        rval = 1;
        break;
      }
       ...
       * functions can be expensive.
       */
      if (!F_ISSET(op, OPT_ALWAYS) && O_VAL(sp, offset) == value)
        break;
      ...
      /* Report to subsystems. */
      if ((op->func != NULL && op->func(sp, spo, sep, &value)) ||
    ...
    case OPT_STR:
       ...
       * functions can be expensive.
       */
      if (!F_ISSET(op, OPT_ALWAYS) && O_STR(sp, offset) != NULL &&
      ...
      /* Report to subsystems. */
      if ((op->func != NULL && op->func(sp, spo, sep, NULL)) ||
 ...
 */
int o_set(SCR *sp, int opt, u_int flags, char *str, u_long val) {
  OPTION *op;

  /* Set a pointer to the options area. */
  op = F_ISSET(&sp->opts[opt], OPT_GLOBAL)
           ? &sp->gp->opts[sp->opts[opt].o_cur.val]
           : &sp->opts[opt];
  ...
  if (LF_ISSET(OS_DEF))
    if (LF_ISSET(OS_STR | OS_STRDUP)) {
      ...
      op->o_def.val = val;
  else if (LF_ISSET(OS_STR | OS_STRDUP)) {
 ...
 */
int opts_empty(SCR *sp, int off, int silent) {
  char *p;

  if ((p = O_STR(sp, off)) == NULL || p[0] == '\0') {
    if (!silent)
      msgq_str(sp, M_ERR, optlist[off].name, "No %s edit option specified");
    return (1);
  }
  return (0);
}
 ...
 */
void opts_dump(SCR *sp, enum optdisp type) {
  OPTLIST const *op;
  ...
  for (numcols = 10; numcols > 1; --numcols) {
    ...
    if (colwidth >= 10) {
      colwidth = (colwidth + STANDARD_TAB) & ~(STANDARD_TAB - 1);
  ...
  for (b_num = s_num = 0, op = optlist; op->name != NULL; ++op) {
    ...
    case OPT_1BOOL:
      ...
      break;
    case OPT_NUM:
      (void)snprintf(nbuf, sizeof(nbuf), "%ld", O_VAL(sp, cnt));
      curlen += strlen(nbuf);
      break;
    case OPT_STR:
    ...
    for (row = 0; row < numrows;) {
      for (base = row, col = 0; col < numcols; ++col) {
        ...
        if ((base += numrows) >= s_num)
          break;
        (void)ex_printf(sp, "%*s", (int)(colwidth - cnt), "");
      }
 ...
 *      Print out an option.
 */
static int opts_print(SCR *sp, OPTLIST const *op) {
  int curlen, offset;
  ...
  case OPT_0BOOL:
  case OPT_1BOOL:
    curlen += ex_printf(sp, "%s%s", O_ISSET(sp, offset) ? "" : "no", op->name);
    break;
  case OPT_NUM:
 ...
 */
OPTLIST const *opts_search(char *name) {
 ...
 */
void opts_nomatch(SCR *sp, char *name) {
  msgq_str(sp, M_ERR, name,
           "set: no %s option: 'set all' gives all option values");
}

static int opts_abbcmp(const void *a, const void *b) {
  return (strcmp(((OABBREV *)a)->name, ((OABBREV *)b)->name));
}

static int opts_cmp(const void *a, const void *b) {
...
int opts_copy(SCR *orig, SCR *sp) {
  ...
  for (cnt = rval = 0; cnt < O_OPTIONCOUNT; ++cnt) {
    if (optlist[cnt].type != OPT_STR || F_ISSET(&optlist[cnt], OPT_GLOBAL))
      continue;
    /*
    ...
    if (O_D_STR(sp, cnt) != NULL &&
        o_set(sp, cnt, OS_DEF | OS_NOFREE | OS_STRDUP, O_D_STR(sp, cnt), 0)) {
    nomem:
