 ...
 */
int editor(GS *gp, int argc, char *argv[]) {
...
#ifdef DEBUG
      "c:D:FlRrSsT:t:vw:", "c:D:eFlRrST:t:w:", "c:D:eFlrST:t:w:"
#else
      "c:FlRrSst:vw:", "c:eFlRrSt:w:", "c:eFlrSt:w:"
#endif /* ifdef DEBUG */
  };

  if (openbsd_pledge("stdio rpath wpath cpath fattr flock getpw tty proc exec",
                     NULL) == -1) {
    perror("pledge");
    goto err;
  }
  ...
  readonly = 0;
  if (!strcmp(bsd_getprogname(), "ex") || !strcmp(bsd_getprogname(), "nex") ||
      !strcmp(bsd_getprogname(), "oex") || !strcmp(bsd_getprogname(), "obex") ||
  ...
  while ((ch = openbsd_getopt(argc, argv, optstr[pmode])) != -1)
    ...
    case 'r': /* Recover. */
      if (flagchk == 't') {
        openbsd_warnx("only one of -r and -t may be specified.");
        return (1);
      }
      flagchk = 'r';
      break;
  ...
  if (secure)
    if (openbsd_pledge("stdio rpath wpath cpath fattr flock getpw tty", NULL) ==
        -1) {
      perror("pledge");
      goto err;
    }
...
void v_end(GS *gp) {
  ...
  while ((mp = LIST_FIRST(&gp->msgq)) != NULL) {
    (void)fprintf(stderr, "%s%.*s", mp->mtype == M_ERR ? "ex/vi: " : "",
                  (int)mp->len, mp->buf);
    LIST_REMOVE(mp, q);
#if defined(DEBUG) || defined(PURIFY)
    free(mp->buf);
 ...
 */
static int v_obsolete(char *argv[]) {
  size_t len;
  char *p;
  ...
  while (*++argv && strcmp(argv[0], "--"))
    if (argv[0][0] == '+') {
      ...
      } else if ((argv[0][1] == 'c' || argv[0][1] == 'T' || argv[0][1] == 't' ||
                  argv[0][1] == 'w') &&
                 argv[0][2] == '\0')
        ++argv;
    }
...
static void attach(GS *gp) {
  ...
  (void)printf("process %ld waiting, enter <CR> to continue: ", (long)getpid());
  (void)fflush(stdout);
