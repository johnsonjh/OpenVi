 ...
 */
int ex_script(SCR *sp, EXCMD *cmdp) {
  /* Vi only command. */
  if (!F_ISSET(sp, SC_VI)) {
    msgq(sp, M_ERR, "The script command is only available in vi mode");
    return (1);
  }
...
static int sscr_init(SCR *sp) {
  ...
  if (sscr_pty(&sc->sh_master, &sc->sh_slave, sc->sh_name, &sc->sh_term,
               &sc->sh_win) == -1) {
...
#else
  if (openpty(&sc->sh_master, &sc->sh_slave, sc->sh_name, &sc->sh_term,
              &sc->sh_win) == -1) {
    msgq(sp, M_SYSERR, "pty");
    goto err;
  }
  ...
  case 0: /* Utility. */
    ...
    if (setenv("TERM", "emacs", 1) == -1 ||
        setenv("TERMCAP", "emacs:", 1) == -1 || setenv("EMACS", "t", 1) == -1)
      _exit(126);
 ...
 */
static int sscr_getprompt(SCR *sp) {
...
more:
  ...
  for (p = t = buf; p < endp; ++p) {
    ...
    if (value == K_CR || value == K_NL) {
      if (db_last(sp, &lline) || db_append(sp, 0, lline, t, p - t))
        goto prompterr;
      t = p + 1;
    }
  }
 ...
 */
int sscr_exec(SCR *sp, recno_t lno) {
  SCRIPT *sc;
  recno_t last_lno;
  ...
  if (sscr_matchprompt(sp, p, len, &tlen)) {
    if (tlen == len) {
    empty:
      msgq(sp, M_BERR, "No command to execute");
      goto err1;
    }
    p += (len - tlen);
    len = tlen;
...
#define MINREAD 1024
  ...
  sc = sp->script;
more:
  switch (nr = read(sc->sh_master, endp, MINREAD)) {
 ...
 *
 */
static int sscr_setprompt(SCR *sp, char *buf, size_t len) {
  SCRIPT *sc;
 ...
 */
static int sscr_matchprompt(SCR *sp, char *lp, size_t line_len, size_t *lenp) {
  SCRIPT *sc;
  size_t prompt_len;
  char *pp;
  ...
  for (pp = sc->sh_prompt; prompt_len && line_len; --prompt_len, --line_len) {
    if (*pp == '\0') {
