 ...
 */
int ex_tag_first(SCR *sp, char *tagarg) {
  ARGS *ap[2], a;
  EXCMD cmd;
 ...
 */
int ex_tag_push(SCR *sp, EXCMD *cmdp) {
  EX_PRIVATE *exp;
  FREF *frp;
  TAG *rtp;
  TAGQ *rtqp, *tqp;
  ...
  cno = sp->cno;
  istmp =
      frp == NULL || (F_ISSET(frp, FR_TMPFILE) && !F_ISSET(cmdp, E_NEWSCREEN));
  ...
  if (F_ISSET(cmdp, E_NEWSCREEN)) {
    ...
    sp = sp->nextdisp;
    exp = EXP(sp);
  } else if (ex_tag_nswitch(sp, TAILQ_FIRST(&tqp->tagq), force))
    goto err;
  ...
  (void)ctag_search(sp, tqp->current->search, tqp->current->slen, tqp->tag);
 ...
 */
int ex_tag_next(SCR *sp, EXCMD *cmdp) {
  EX_PRIVATE *exp;
  TAG *tp;
  TAGQ *tqp;
 ...
 */
int ex_tag_prev(SCR *sp, EXCMD *cmdp) {
  EX_PRIVATE *exp;
  TAG *tp;
  TAGQ *tqp;
 ...
 */
int ex_tag_nswitch(SCR *sp, TAG *tp, int force) {
 ...
 */
int ex_tag_pop(SCR *sp, EXCMD *cmdp) {
  EX_PRIVATE *exp;
  TAGQ *tqp, *dtqp = NULL;
  size_t arglen;
  long off;
  ...
  case 1: /* Name or number. */
    ...
    if (tqp == NULL) {
      msgq(sp, M_ERR,
           "Less than %s entries on the tags stack; use :display t[ags]", arg);
      return (1);
    }
    dtqp = tqp;
    break;
 ...
 */
int ex_tag_top(SCR *sp, EXCMD *cmdp) {
  EX_PRIVATE *exp;
 ...
 */
static int tag_pop(SCR *sp, TAGQ *dtqp, int force) {
  EX_PRIVATE *exp;
  TAG *tp;
  TAGQ *tqp;
  ...
  TAILQ_FOREACH(tqp, &exp->tq, q) {
    ...
    TAILQ_FOREACH(tp, &tqp->tagq, q) {
      ...
      if ((len = strlen(p)) > L_NAME) {
        len = len - (L_NAME - 4);
        (void)ex_printf(sp, "   ... %*.*s", L_NAME - 4, L_NAME - 4, p + len);
      } else
        (void)ex_printf(sp, "   %*.*s", L_NAME, L_NAME, p);
      if (tqp->current == tp)
        (void)ex_printf(sp, "*");
      ...
      if (tp == TAILQ_FIRST(&tqp->tagq) && tqp->tag != NULL &&
        ...
        (void)ex_printf(sp, "%s%.*s", tqp->current == tp ? "    " : "     ",
                        (int)len, tqp->tag);
      }
      (void)ex_printf(sp, "\n");
    }
 ...
 */
int ex_tag_copy(SCR *orig, SCR *sp) {
 ...
 */
static int tagf_copy(SCR *sp, TAGF *otfp, TAGF **tfpp) {
  TAGF *tfp;
 ...
 */
static int tagq_copy(SCR *sp, TAGQ *otqp, TAGQ **tqpp) {
  TAGQ *tqp;
  size_t len;
 ...
 */
static int tag_copy(SCR *sp, TAG *otp, TAG **tpp) {
  TAG *tp;
  size_t len;
 ...
 */
static int tagf_free(SCR *sp, TAGF *tfp) {
  EX_PRIVATE *exp;
 ...
 */
void tag_msg(SCR *sp, tagmsg_t msg, char *tag) {
  switch (msg) {
  case TAG_BADLNO:
 ...
 */
int ex_tagf_alloc(SCR *sp, char *str) {
  EX_PRIVATE *exp;
  TAGF *tfp;
  size_t len;
  char *p, *t;
 ...
 */
static int ctag_search(SCR *sp, char *search, size_t slen, char *tag) {
  MARK m;
  char *p;
  ...
  if (isdigit(search[0])) {
    ...
    m.lno = 1;
    m.cno = 0;
    if (f_search(sp, &m, &m, search, slen, NULL, SEARCH_FILE | SEARCH_TAG)) {
 ...
 */
static TAGQ *ctag_slist(SCR *sp, char *tag) {
  EX_PRIVATE *exp;
  TAGF *tfp;
  TAGQ *tqp;
  size_t len;
  ...
  if (TAILQ_EMPTY(&tqp->tagq)) {
    ...
    if (echk)
      TAILQ_FOREACH(tfp, &exp->tagfq, q)
    if (F_ISSET(tfp, TAGF_ERR) && !F_ISSET(tfp, TAGF_ERR_WARN)) {
      errno = tfp->errnum;
 ...
 */
static int ctag_sfile(SCR *sp, TAGF *tfp, TAGQ *tqp, char *tname) {
  struct stat sb;
  TAG *tp;
   ...
   */
  if (fstat(fd, &sb) != 0 ||
      (map = mmap(NULL, (size_t)sb.st_size, PROT_READ | PROT_WRITE, MAP_PRIVATE,
  ...
  for (;;) {
    ...
    *p = '\0';
    ...
    CALLOC_GOTO(sp, tp, 1, sizeof(TAG) + dlen + 2 + nlen + 1 + slen + 1);
    tp->fname = tp->buf;
    if (dlen != 0) {
 ...
 */
static void ctag_file(SCR *sp, TAGF *tfp, char *name, char **dirp,
                      size_t *dlenp) {
  struct stat sb;
  char *p, buf[PATH_MAX];
   ...
   */
  *dlenp = 0;
  if (name[0] != '/' && stat(name, &sb) &&
      (p = strrchr(tfp->name, '/')) != NULL) {
    *p = '\0';
 ...
 */
static char *linear_search(char *string, char *front, char *back, long tl) {
  char *end;
 ...
 */
static int compare(char *s1, char *s2, char *back) {
