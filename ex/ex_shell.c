 ...
 */
int ex_shell(SCR *sp, EXCMD *cmdp) {
  int rval;
  char buf[PATH_MAX];
 ...
 */
int ex_exec_proc(SCR *sp, EXCMD *cmdp, char *cmd, const char *msg,
                 int need_newline) {
  GS *gp;
  const char *name;
  pid_t pid;
 ...
 */
int proc_wait(SCR *sp, pid_t pid, const char *cmd, int silent, int okpipe) {
  size_t len;
  int nf, pstat;
  char *p;
  ...
  if (WIFSIGNALED(pstat) && (!okpipe || WTERMSIG(pstat) != SIGPIPE)) {
    ...
    len = strlen(p);
    msgq(sp, M_ERR, "%.*s%s: received signal: %s%s", MINIMUM(len, 20), p,
         len > 20 ? " ..." : "", strsignal(WTERMSIG(pstat)),
#ifdef WCOREDUMP
  ...
  if (WIFEXITED(pstat) && WEXITSTATUS(pstat)) {
    ...
    if (!silent) {
      ...
      p = msg_print(sp, cmd, &nf);
      len = strlen(p);
      msgq(sp, M_ERR, "%.*s%s: exited with status %d", MINIMUM(len, 20), p,
           len > 20 ? " ..." : "", WEXITSTATUS(pstat));
      if (nf)
        FREE_SPACE(sp, p, 0);
    }
    return (1);
