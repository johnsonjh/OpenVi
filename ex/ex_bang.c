...
int ex_bang(SCR *sp, EXCMD *cmdp) {
    ...
    if (sp->ep != NULL && F_ISSET(sp->ep, F_MODIFIED)) {
      if (O_ISSET(sp, O_AUTOWRITE)) {
        if (file_aw(sp, FS_ALL))
          return (0);
      } else if (O_ISSET(sp, O_WARN) && !F_ISSET(sp, SC_EX_SILENT))
        msg = "File modified since last write.";
    }

    /* If we're still in a vi screen, move out explicitly. */
    (void)ex_exec_proc(sp, cmdp, ap->bp, msg,
                       !F_ISSET(sp, SC_EX | SC_SCR_EXWROTE));
  }
   ...
   */
    ...
    rval = ex_filter(sp, cmdp, &cmdp->addr1, &cmdp->addr2, &rm, ap->bp, ftype);
    (void)rval;
